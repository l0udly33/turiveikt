...
public class Tank {
	...
	public static float maxVel = 1.5f, maxAcc = 0.3f,rotSpeed = 0.025f,shootVel=2f,maxHealth=10f;
	...
	public Tank(Vector pos, int w, int h, Color c1, Color c2, TankStrategy strategy) {
		this.pos = pos;
		this.w = dim;
		this.h = dim;
		this.color1 = c1;
		...
		// Set tank attributes based on the strategy
		maxVel = strategy.getMaxVelocity();
		maxAcc = strategy.getMaxAcceleration();
		shootVel = strategy.getShootingVelocity();
		maxHealth = strategy.getMaxHealth();
		shootingCoolDown = strategy.getShootingCooldown();
...
//		}
	...
	public void increaseHealth() {
		if (health < maxHealth) {
			health += 5f; // Restore 20 health (adjust the amount as needed)
			if (health > maxHealth) {
